...
// Shims for helpers that exist in newer pokeemerald-expansion versions
// (miriamlefae/pokeemerald-expansion) but not yet in this fork. Remove these
// if/when the underlying fork gets the real implementations.
#define errorf(fmt, ...) ((void)0)

static inline s32 SubtractClamped(s32 lowestVal, s32 highestVal, s32 currentVal, s32 delta)
{
...
// TODO: placeholder icon (duplicate of debug.png) until custom art is made — see CLAUDE.md.
static const u32 sQuestIconGfx[] = INCBIN_U32("graphics/unbound_start_menu/sprites/quest.4bpp.smol");
// Icona reale dell'oggetto Box Portatile, ricolorata sulla palette icons.pal e
// centrata in un frame 32x32 (l'originale e' 24x24) — vedi CLAUDE.md.
static const u32 sBoxIconGfx[] = INCBIN_U32("graphics/unbound_start_menu/sprites/box.4bpp.smol");
...
static bool32 UsmMenuCB_Quest(u32 state);
static bool32 UsmMenuCB_Box(u32 state);
...
static const struct Usm_MenuItem sUsmMenuItems[USM_ICO_COUNT] = {
    ...
    [USM_ICO_QUEST]   = USM_MENU_ITEM(Quest, "Quests"),
    [USM_ICO_BOX]     = USM_MENU_ITEM(Box, "Box"),
...
static bool32 UsmMenuCB_Quest(u32 state)
{
    ...
    default:
        if (!gPaletteFade.active) {
            PlayRainStoppingSoundEffect();
            CleanupOverworldWindowsAndTilemaps();
            QuestMenu_Init(CB2_ReturnToFieldWithOpenMenu);
            return TRUE;
        }
    }
    return FALSE;
...
static bool32 UsmMenuCB_Box(u32 state)
{
    // ShowPokemonStorageSystemPC va invocata solo da uno script
    // (lockall/waitstate/releaseall): chiamarla direttamente da qui
    // lascia il campo in uno stato inconsistente all'uscita. Si
    // riusa lo stesso script del PC fisico/oggetto Box Portatile.
    //
    // Il menu Start (USM) non cambia mai il main callback: e' un overlay
    // disegnato sopra il campo (vedi Usm_InitStartMenu, che non chiama mai
    // SetMainCallback2 tranne nel caso di errore OOM). Restiamo quindi
    // sempre nel CB2 di campo, esattamente come quando si interagisce con
    // un PC fisico — percio' non serve alcun CB2_ReturnToField ne' fade:
    // basta avviare lo script, partira' da solo al prossimo frame.
...
static void Usm_ShowSafariText(void)
{
    ...
    ConvertIntToDecimalStringN(gStringVar1, GetSafariZoneStepCounter(), STR_CONV_MODE_LEFT_ALIGN, 3);
...
static const enum Usm_Icons sUsmDefaultItems[] = {
    ...
    USM_ICO_POKENAV, USM_ICO_DEXNAV, USM_ICO_QUEST, USM_ICO_BOX, USM_ICO_TRAINER, USM_ICO_SAVE,
...
static void Usm_InsertSavedItem(enum Usm_Icons item)
{
    ...
    // This fork has no assertf.h; guard for real instead of asserting, since an
    // assert would just get compiled out anyway and this array write must not
    // run out of bounds on a shipped ROM.
    if (saved->count >= arrayCount)
        return;
...
static bool32 Usm_IsItemAvailable(enum Usm_Icons item)
{
    switch (item) {
        ...
        case USM_ICO_QUEST:   return FlagGet(FLAG_QUEST_MENU_ACTIVE);
        case USM_ICO_BOX:     return FlagGet(FLAG_SYS_BOX_LINK_GET);
...
static void Usm_CreateIcons(s16 x, s16 y)
{
    ...
    for (u32 i = 0; i < count; i++) {
        ...
        // NOTE: this fork's Sprite struct has no copyToObjWin (OBJ WINDOW copy)
        // field, so icons won't render correctly inside Flash's darkness circle
        // yet (see CLAUDE.md, "Unbound Style Start Menu" section).
        // if (Usm_IsFlashObscured())
...
static u32 Usm_CreateArrowSprite(s16 x, s16 y, bool32 flip)
{
    ...
    // if (Usm_IsFlashObscured())
    //     gSprites[spriteId].copyToObjWin = TRUE; // see note in Usm_CreateIcons
